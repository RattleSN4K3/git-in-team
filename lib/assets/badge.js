(function(){
  ...
  // search for a script tag pointing to gitinteam.js
  function search(){
    var replaced = 0;
    ...
    for (var i = 0; i < scripts.length; i++) {
      ...
      if (/\/gitinteam\.js(\?.*)?$/.test(script.src)) {
        // replace script with iframe
        replace(script);

        // we abort the search for subsequent
        // gitinteam.js executions to exhaust
        // the queue
        return true;
      }
    }
  ...
  function replace(script){
    ...
    iframe.src = script.src.replace(/\/gitinteam\.js.*/, iframePath);
    iframe.style.borderWidth = 0;
    iframe.className = '__gitinteam';
    ...
    iframe.style.width = (large ? 171 : 136) + 'px';
  ...
  function setup(iframe){
    ...
    iframe.contentWindow.postMessage('gitinteam:' + id, '*');
    window.addEventListener('message', function(e){

      // show dialog upon click
      if ('gitinteam-click:' + id  == e.data) {
        showDialog(iframe, id);
      }

      // update width
      var wp = 'gitinteam-width:' + id + ':';
      ...
      // resize dialog upon loaded iframe page
      if (/^gitinteam-height:/.test(e.data)) {
        var height = e.data.replace(/^gitinteam-height:/, '');
        var el = document.getElementById("dialog_"+id);
        if (el != null){
  ...
  function showDialog(iframe, id){
    ...
    div.appendChild(a2);
    div.appendChild(ni);
    var divcontainer = iframe.parentNode;
    divcontainer.insertBefore(div, iframe);
    ...
    function click(ev){
      if ('__gitinteam' != ev.target.className) {
        hide();
      }
    }
